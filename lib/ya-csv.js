 ...
 *     - quoteAll {boolean} - quote every field regardless of content.
 *       Defaults to true. Pass false to only quote fields that need it
 *       (contain the separator, quotechar, escapechar, or a CR/LF).
 */
...
function _writeArray(writer, arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        var field = _stringifyField(arr[i]);
        var quoted = writer.quoteAll || _fieldNeedsQuoting(writer, field);
        if (quoted) out.push(writer.quotechar);
        _appendField(out, writer, field);
        if (quoted) out.push(writer.quotechar);
    }
    out.push("\r\n");
...
function _stringifyField(field) {
    if (typeof field === 'string') return field;
    if (typeof field === 'undefined' || field === null) return '';
    return String(field);
}

// Whether a field needs to be wrapped in quotechar to round-trip correctly:
// it contains the separator, the quotechar, a CR/LF, or (see parse()'s
// escape handling) the escapechar - a raw escapechar left unquoted can be
// silently swallowed on read-back if it doesn't pair up with a following
// quotechar/escapechar. An empty field never needs quoting.
function _fieldNeedsQuoting(writer, field) {
    if (field === '') return false;
    if (field.indexOf(writer.separator) !== -1) return true;
    if (writer.quotechar && field.indexOf(writer.quotechar) !== -1) return true;
    if (writer.escapechar && field.indexOf(writer.escapechar) !== -1) return true;
...
function _setOptions(obj, options) {
    ...
    // Default true for backward compatibility: every field gets wrapped in
    // quotechar, same as always. Pass quoteAll: false to only quote fields
    // that actually need it (contain the separator, quotechar, escapechar,
    // or a CR/LF).
    obj.quoteAll = (typeof options.quoteAll !== 'undefined') ? options.quoteAll : true;
